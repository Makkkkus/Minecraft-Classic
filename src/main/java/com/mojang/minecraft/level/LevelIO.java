...
public final class LevelIO {
	...
	public final Level loadOnline(String host, String levelName, int id) {
		if (this.progressBar != null) {
		...
		try {
			if (progressBar != null) {
				progressBar.setText("Connecting..");
			}

			HttpURLConnection connection = (HttpURLConnection) new URL("http://" + host + "/level/load.html?id=" + id + "&user=" + levelName).openConnection();
			...
			if (stream.readUTF().equalsIgnoreCase("ok")) {
				return this.load(stream);
			} else {
				if (this.progressBar != null) {
					this.progressBar.setText("Failed: " + stream.readUTF());
				}
